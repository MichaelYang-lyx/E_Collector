...
import { type GetContractEventsParameters, type GetContractEventsReturnType } from './public/getContractEvents.js';
...
export type GetContractParameters<TTransport extends Transport = Transport, TChain extends Chain | undefined = Chain | undefined, TAccount extends Account | undefined = Account | undefined, TAbi extends Abi | readonly unknown[] = Abi, TPublicClient extends Client<TTransport, TChain> | unknown = unknown, TWalletClient extends Client<TTransport, TChain, TAccount> | unknown = unknown, TAddress extends Address = Address> = {
    /** Contract ABI */
    abi: TAbi;
    /** Contract address */
    address: TAddress;
...
export type GetContractReturnType<TAbi extends Abi | readonly unknown[] = Abi, TPublicClient extends Client | unknown = unknown, TWalletClient extends Client | unknown = unknown, TAddress extends Address = Address, _EventNames extends string = TAbi extends Abi ? Abi extends TAbi ? string : ExtractAbiEventNames<TAbi> : string, _ReadFunctionNames extends string = TAbi extends Abi ? Abi extends TAbi ? string : ExtractAbiFunctionNames<TAbi, 'pure' | 'view'> : string, _WriteFunctionNames extends string = TAbi extends Abi ? Abi extends TAbi ? string : ExtractAbiFunctionNames<TAbi, 'nonpayable' | 'payable'> : string, _Narrowable extends boolean = IsNarrowable<TAbi, Abi>> = Prettify<(TPublicClient extends Client ? (IsNever<_ReadFunctionNames> extends true ? unknown : {
    /**
     ...
     */
    getEvents: {
        [EventName in _EventNames]: GetEventsFunction<_Narrowable, TAbi, EventName>;
    };
    /**
...
}) : unknown) & (TWalletClient extends Client ? IsNever<_WriteFunctionNames> extends true ? unknown : {
    /**
     * Estimates the gas necessary to complete a transaction without submitting it to the network.
     *
     * @example
...
type GetEventsFunction<Narrowable extends boolean, TAbi extends Abi | readonly unknown[], TEventName extends string, TAbiEvent extends AbiEvent = TAbi extends Abi ? ExtractAbiEvent<TAbi, TEventName> : AbiEvent, Args = AbiEventParametersToPrimitiveTypes<TAbiEvent['inputs']>, Options = Prettify<Omit<GetContractEventsParameters<TAbi, TEventName>, 'abi' | 'address' | 'args' | 'eventName'>>, IndexedInputs = Extract<TAbiEvent['inputs'][number], {
    indexed: true;
}>> = Narrowable extends true ? (...parameters: IsNever<IndexedInputs> extends true ? [options?: Options] : [args?: Args, options?: Options]) => Promise<GetContractEventsReturnType<TAbi, TEventName>> : (...parameters: [options?: Options] | [
